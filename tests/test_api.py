...
def test_healthz_answers_even_when_the_database_is_down(client, monkeypatch) -> None:
    ...
    The failure is injected at the engine, with no socket involved. Two
    earlier versions of this test were environment-dependent: one assumed no
    database was listening on the default port (passed locally, failed in CI
    where PostGIS is up), and one dialled an unreachable port (which hung for
    minutes on a runner that drops such packets rather than refusing them).
    A test about a database being down should not need a network at all.
    """
    import bob_sentinel.db as db_module

    def refuse() -> None:
        raise OperationalError("SELECT PostGIS_Version()", {}, OSError("connection refused"))

    monkeypatch.setattr(db_module, "get_engine", refuse)
